...
public class Utils {
    ...
    /*public static void dump_json_file(String json_data, String json_file_abs_path){
        ...
        try {
            ...
            logger.error("需要创建的文件路径是：" + json_file_abs_path);
            e.printStackTrace();
        }
    }*/
        ...
        try{
            for(String key : keys){
                if(json_content instanceof JSONArray){
                    ...
                    logger.error(String.format("invalid type value: %s , the class is :%s .",json_content,json_content.getClass()));
                    logger.error(String.format("resp解析错误，其中需要解析的response内容是 %s，请检查case内容是否正确.",query));
                    break ;
                }
            }
        }catch(Exception e){
